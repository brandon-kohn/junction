...
struct GrampaStats {
 ...
 TURF_TRACE_DECLARE(JUNCTION_API, Grampa, 37)
...
struct Grampa {
    ...
    static void garbageCollectTable(Table* table) {
        TURF_ASSERT(table);
        DefaultQSBR().enqueue_mem_fun(&Table::destroy, table);
    }

    static void garbageCollectFlatTree(FlatTree* flatTree) {
        TURF_ASSERT(flatTree);
        DefaultQSBR().enqueue_mem_fun(&FlatTree::destroy, flatTree);
    }
...
endMigration:
    ...
    DefaultQSBR().enqueue_mem_fun(&TableMigration::destroy, this);
}
...
void Grampa<Map>::FlatTreeMigration::run() {
    ...
    DefaultQSBR().enqueue_mem_fun(&FlatTreeMigration::destroy, this);
}
